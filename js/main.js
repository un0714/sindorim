...
async function onCreate(props) {
  const phone = document.getElementById(props === 'pc' ? 'userTel' : 'userMobileTel').value;
  const platform = document.querySelector(props === 'pc' ? 'input[name="radioMobile"]:checked' : 'input[name="mo_radioMobile"]:checked').value;
  const reserveTag = document.getElementById(props === 'pc' ? 'goReservation' : 'goReservation2');
  const personal = document.getElementById(props === 'pc' ? 'checkPersonal' : 'checkPersonalMo').checked;
  const eventPersonal = document.getElementById(props === 'pc' ? 'checkEvent' : 'checkEventMo').checked;
  ...
  if (!eventPersonal) {
    alert('이벤트 등 프로모션 알림 수신에 동의해주세요.');
    return false;
  }

  const url = `https://live-kor-sindorim.monstergames.kr/kor_live_game/revservation?phone=${phone}&platform=${platform}`;
  await fetch(url)
    .then(response => {
      if (response.status == 200) {
        alert('사전등록되었습니다.');
        let promise = new Promise(function (resolve, reject) {
          location.href = '#reservationsub';
...
async function loadCount() {
  const url = 'https://live-kor-sindorim.monstergames.kr/kor_live_game/revservationcount';
  await fetch(url)
    .then(response => {
      return response.json();
    })
    .then(function (data) {
      // reCount = data.totalCount;
      const reCount = data.totalCount;
      countNumber(reCount);
      levelBar(reCount);
    })
    .catch(error => console.log('error:', error));
...
function countNumber(reCount) {
  if (reCount) {
    countBoxPc.innerHTML = new Intl.NumberFormat().format(reCount);
    countBoxMo.innerHTML = new Intl.NumberFormat().format(reCount);
  }
}
countNumber();
